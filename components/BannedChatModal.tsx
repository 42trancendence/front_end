...
export default function BannedChatModal({
	...
	return userData.map((user: any, index: number) => (
		...
		<Menu as="li" key={index}>
			...
			<Menu.Button className={clsx(!user.isEntered && "opacity-50", "group flex w-full items-center gap-x-4 rounded-md p-2 text-sm font-normal leading-6 text-indigo-200 hover:bg-zinc-700 hover:text-white")}>
			<img
